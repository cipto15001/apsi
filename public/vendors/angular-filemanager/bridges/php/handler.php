...
if (Request::getApiParam('action') === 'list') {
	...
	$list = CallAPI('GET', 'http://localhost:8000/api/file_manager/list', ['path' => $path]);
	echo $list;
	exit;
...
if (Request::getApiParam('action') === 'rename') {
    ...
    $response = CallAPI('GET', 'http://localhost:8000/api/file_manager/rename?item=' . $item . '&newItemPath=' . $newItemPath);
	ChromePhp::log($item);
	echo $response;
	exit;
...
if (Request::getApiParam('action') === 'move') {
    ...
    foreach($items as $item) {
		...
		$response = CallAPI('GET', "http://localhost:8000/api/file_manager/move?item=$item&finalPath=$finalPath");
		/*
...
if (Request::getApiParam('action') === 'remove') {
    $items = Request::getApiParam('items');
	foreach ($items as $item) {		
		ChromePhp::log(gettype($item));
		$response = CallAPI('GET', 'http://localhost:8000/api/file_manager/remove?item=' . $item);
	}
	echo $response;
	exit;
    /*
...
if (Request::getApiParam('action') === 'createFolder') {
    $newPath = Request::getApiParam('newPath');
    $response = CallAPI('GET', 'http://localhost:8000/api/file_manager/createFolder?newPath=' . $newPath);
	echo $response;
	exit;
	/*
...
if (Request::getApiParam('action') === 'compress') {
	...
	foreach ($items as $item) {
		CallAPI('GET', "http://localhost:8000/api/file_manager/appendCompressDir?item=$item");
	}
	$response = CallAPI('GET', "http://localhost:8000/api/file_manager/compress?destination=$destination&compressedFilename=$compressedFilename");
