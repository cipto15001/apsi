...
include __DIR__ . '/includes/ChromePhp.php';
...
if (Request::getApiParam('action') === 'list') {
    ...
    $id = $_GET['id'];
    $response = CallAPI('GET', "http://localhost:8000/api/file_manager/list?id=$id&path=$path");
    ChromePhp::log("response is " . $response);
    echo $response;
    exit;
}
...
if (Request::getApiParam('action') === 'rename') {
    ...
    $id = $_GET['id'];
	ChromePhp::log("item is $item and newItempath is $newItemPath");
    $response = CallAPI('GET', "http://localhost:8000/api/file_manager/rename?id=$id&item=$item&newItemPath=$newItemPath");
	echo $response;
	exit;
...
if (Request::getApiParam('action') === 'move') {
    ...
    foreach($items as $item) {
		...
		$response = CallAPI('GET', "http://localhost:8000/api/file_manager/move?id=$id&item=$item&finalPath=$finalPath");
		/*
...
if (Request::getApiParam('action') === 'remove') {
    ...
    $id = $_GET['id'];
	foreach ($items as $item) {		
		$response = CallAPI('GET', "http://localhost:8000/api/file_manager/remove?id=$id&item=$item");
	}
	echo $response;
	exit;
    /*
...
if (Request::getApiParam('action') === 'createFolder') {
    $newPath = Request::getApiParam('newPath');
    $id = $_GET['id'];
    $response = CallAPI('GET', "http://localhost:8000/api/file_manager/createFolder?id=$id&newPath=$newPath");
	echo $response;
	exit;
	/*
...
if (Request::getApiParam('action') === 'compress') {
    ...
    ChromePhp::log($compressedFilename);
    $id = $_GET['id'];

    $itemParam = "";

	foreach ($items as $item) {
        $itemParam .= "items[]=$item&";
    }

    $itemParam = rtrim($itemParam, "&");
    
	$response = CallAPI('GET', "http://localhost:8000/api/file_manager/compress?id=$id&destination=$destination&compressedFilename=$compressedFilename&$itemParam");
...
if (Request::getApiParam('action') === 'extract') {
    $item = Request::getApiParam('item');
    $destination = "." . Request::getApiParam('destination');
    $folderName = Request::getApiParam('folderName');
    $id = $_GET['id'];
 
    ChromePhp::log("$item $destination $folderName $id");
	$response = CallAPI('GET', "http://localhost:8000/api/file_manager/extract?item=$item&destination=$destination&folderName=$folderName&id=$id");
    exit;
}
