...
var WorkerPool = function () {
  function WorkerPool(createWorker) {
    var maxThreads = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 2;
  ...
  _createClass(WorkerPool, [{
    ...
    value: function exec() {
      ...
      return new Promise(function (res, rej) {
        // if there are no free workers, add to queue
        _this._queue.push(['exec', args, res]);
      });
    }
  }, {
    key: 'postMessage',
    value: function postMessage() {
      ...
      var worker = this.getFreeWorkerOrCreate();
      if (worker) {
        console.log('exec directly');
        return this._exec(worker, 'postMessage', args);
      }

      console.log('add to queue', worker);

      return new Promise(function (res, rej) {
        // if there are no free workers, add to queue
        _this2._queue.push(['postMessage', args, res]);
      });
    }
  }, {
    key: '_exec',
    ...
    value: function _waitAndRemoveWorkers(workers) {
      ...
      // one worker should be alive always
      if (this._workers.length - workers.length < 1) {
        workers.pop();
      }

      setTimeout(function () {
        workers.forEach(function (worker) {
          return worker.isFree() ? _this4._removeWorker(worker) : null;
        });
      }, 5000);
    }
  }, {
    ...
    value: function create(opts) {
      return new WorkerPool(function () {
