...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">1×</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">const TinyEmitter = require('./tiny-emitter');
&nbsp;
...
&nbsp;
  ...
  exec(operationName, data = null, transferable = [], onEvent) {
    return new Promise((res, rej) =&gt; {
      ...
      this._worker.postMessage([messageId, data, operationName], transferable || <span class="branch-1 cbranch-no" title="branch not covered" >[])</span>;
    });
  }
&nbsp;
  /**
  ...
  postMessage(data = null, transferable = [], onEvent) {
    return new Promise((res, rej) =&gt; {
      const messageId = this._messageId++;
      this._messages.set(messageId, [res, rej, onEvent]);
      this._worker.postMessage([messageId, data], transferable || <span class="branch-1 cbranch-no" title="branch not covered" >[])</span>;
    });
  }
&nbsp;
  emit(eventName, ...args) {
...
&nbsp;
    if(type === MESSAGE_EVENT)
      this._onEvent(...args);
    else <span class="missing-if-branch" title="else path not taken" >E</span>if(type === MESSAGE_RESULT)
      this._onResult(...args);
    else
<span class="cstat-no" title="statement not covered" >      throw new Error(`Wrong message type '${type}'`);</span>
  }
&nbsp;
